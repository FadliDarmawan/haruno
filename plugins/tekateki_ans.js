...
handler.before = async function (m) {
    ...
    if (/^(me)?nyerah$/i.test(m.text)) {
        await this.sendButton(m.chat, `Jawabannya adalah ${JSON.parse(JSON.stringify(this.tekateki[id][1].jawaban))}`.trim(), '© Haruno', 'TEKA TEKI', ',tekateki').then(() => {
    ...
    if (m.text.toLowerCase() == json.jawaban.toLowerCase().trim()) {
        ...
        await this.sendButton(m.chat, `*Benar!* +${this.tekateki[id][2]} XP`, '© Haruno', 'TEKA TEKI', '.tekateki')
