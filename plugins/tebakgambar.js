...
Bonus: ${poin} XP
    `.trim()
  conn.tebakgambar[id] = [
    await conn.sendButtonImg(m.chat, caption, await (await fetch(json.img)).buffer(), '© Haruno', 'BANTUAN', '.hint')
    ,
    json, poin,
    setTimeout(async () => {
      if (conn.tebakgambar[id]) await conn.sendButton(m.chat, `Waktu habis!\nJawabannya adalah *${json.jawaban}*`, '© Haruno', 'TEBAK GAMBAR', '.tebakgambar')
