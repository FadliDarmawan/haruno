...
handler.before = async function (m) {
    ...
    if (m.isGroup && chat.expired != 0) {
        if (new Date() * 1 >= chat.expired) {
            let name = conn.getName(m.chat)
            await conn.sendButtonImg(m.chat, await(await fetch(thumbfoto)).buffer(), `Waktu bot di ${name} sudah habis. Sudah saatnya ${conn.user.name} keluar dari group. Terimakasih`, watermark, 'Sayounara', '.')
